// The structure of a Arch package looks something like this:
//
// foobar_1.2.3_i386.pkg.tar.xz   # Actually an ar archive
//     .PKGINFO        # A nearly identicle copy of PKGBUILD file used to create package
//     .MTREE          # generates a map of a directory structure with attributes included
...
// metadata, as well as generating the md5sums file.
...
pub fn bundle_project(settings: &Settings) -> crate::Result<Vec<PathBuf>> {
  ...
  let package_name = format!("{}.pkg", package_base_name);
  common::print_bundling(&package_name)?;
  let base_dir = settings.project_out_directory().join("bundle/arch");
  ...
  let package_dir = generate_folders(settings, &package_dir).chain_err(|| "Failed to build folders")?;
  // Generate PKGBUILD file.
  generate_pkgbuild_file(settings, arch, &package_dir)
    .chain_err(|| "Failed to create PKGBUILD file")?;
  generate_md5sums(&package_dir).chain_err(|| "Failed to append md5sums to PKGBUILD file")?;


  // Apply tar/gzip/ar to create the final package file.
  let package_tar_gz_path =
    tar_and_gzip_dir(package_dir).chain_err(|| "Failed to tar/gzip package directory")?;
  create_archive(
    vec![package_dir_tar_gz_path],
    &package_path,
  )
...
pub fn generate_folders(settings: &Settings, package_dir: &Path) -> crate::Result<PathBuf> {
  ...
  // Generate data files.
  let binary_dest = package_dir.join("usr/bin").join(settings.binary_name());
  common::copy_file(settings.binary_path(), &binary_dest)
    .chain_err(|| "Failed to copy binary file")?;
  transfer_resource_files(settings, &package_dir).chain_err(|| "Failed to copy resource files")?;
  generate_icon_files(settings, &package_dir).chain_err(|| "Failed to create icon files")?;
  generate_desktop_file(settings, &package_dir).chain_err(|| "Failed to create desktop file")?;
...
fn generate_pkgbuild_file(
  ...
  writeln!(&mut file, "pkgver={}", settings.version_string())?;
  writeln!(&mut file, "pkgrel=1", settings.version_string())?;
  ...
  writeln!(&mut file, "pkgdesc={}", short_description)?;
  ...
  writeln!(&mut file, "arch=('any')")?;
  if !settings.homepage_url().is_empty() {
    writeln!(&mut file, "url={}", settings.homepage_url())?;
  }
  writeln!(&mut file, "license=('unknown')")?;
...
fn generate_md5sums(package_dir: &Path) -> crate::Result<()> {
  let md5sums_path = data_dir.join("md5sums");
...
/// `package_dir`.
fn transfer_resource_files(settings: &Settings, package_dir: &Path) -> crate::Result<()> {
  let resource_dir = package_dir.join("usr/lib").join(settings.binary_name());
...
fn generate_icon_files(settings: &Settings, package_dir: &PathBuf) -> crate::Result<()> {
